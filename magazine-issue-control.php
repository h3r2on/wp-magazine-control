...
function magazine_issue_check_override(){
    ...
    // nedd to setup a way to setup a better way to list valid versions and not setup the constant unless it's good
    $good_versions = array(1,2,3);
    if (isset($_GET['issue']) && is_numeric($_GET['issue'])) {
        $override = $_GET['issue'];
        if (in_array($override,$good_versions)) {
...
function magazine_beta_mode($query) {
    if(defined('MIC_BETA')) {
        $query->set('post_status', 'beta');
    }
}
add_action('pre_get_posts','magazine_beta_mode');


function beta_post_status(){
  register_post_status( 'beta', array(
      'label' => _x( 'Beta', 'post' ),
      'public' => false,
      'exclude_from_search' => false,
      'show_in_admin_all_list' => true,
      'show_in_admin_status_list' => true,
      'label_count'               => _n_noop( 'Beta <span class="count">(%s)</span>', 'Beta <span class="count">(%s)</span>' ),
  ) );
}
add_action( 'init', 'beta_post_status' );


if ( is_home() ) {
    function magazine_issue_show_posts( $query ) {
        $cur_issue = get_option('mic_current_issue');
